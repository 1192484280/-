...
#import "SubmitStartController.h"
#import "SubmitApprovalingController.h"
#import "SubmitRefuseController.h"
#import "SubmitCompleteController.h"
#import "SearchView.h"
#import "SMPagerTabView.h"
#import "ReimbursApprovalDetailController.h"

@interface SubmitViewController ()<SMPagerTabViewDelegate,SubmitStartControllerDelegate,SubmitApprovalingControllerDelegate,SubmitRefuseControllerDelegate,SubmitCompleteControllerDelegate,SearchViewDelegate>
...
- (void)ClickSearchBtn:(NSString *)title{
    ...
    if (self.pageNum == 0) {
        ...
        noti = SubmitStartNotification;
        
    }else if(self.pageNum == 1){
        
        noti = SubmitApprovingtNotification;
    }else if(self.pageNum == 2){
        
        noti = SubmitRefuseNotification;
    }else{
        
        noti = SubmitCompleteNotification;
    }
...
- (void)setUI{
    ...
    SubmitStartController *vc1 = [[SubmitStartController alloc]initWithNibName:nil bundle:nil];
    vc1.delegate = self;
    vc1.title = @"发起";
    
    SubmitApprovalingController *vc2 = [[SubmitApprovalingController alloc]initWithNibName:nil bundle:nil];
    vc2.delegate = self;
    vc2.title = @"审批中";
    
    SubmitRefuseController *vc3 = [[SubmitRefuseController alloc]initWithNibName:nil bundle:nil];
    vc3.delegate = self;
    vc3.title = @"已拒绝";
    
    SubmitCompleteController *vc4 = [[SubmitCompleteController alloc]initWithNibName:nil bundle:nil];
    vc4.delegate = self;
    vc4.title = @"完成";
...
- (void)selectedCell:(BOOL)showBottomView2{
    ...
    VC.showBottomView2 = showBottomView2;
